<form action="">
   <fieldset>
      <legend>Testing Wilayah</legend>

      <p>
         <select name="propinsi" id="propinsi" style="width: 30%;">
            <option value="">PILIH PROPINSI</option>
            <?php foreach($propinsi->result() as $prov): ?>
            <option value="<?php echo $prov->id_propinsi; ?>"><?php echo $prov->nama; ?></option>
            <?php endforeach; ?>
         </select>
      </p> 
...
<script>
   ...
   $("#kota").remoteChained({
      parents : "#propinsi",
      url : "<?php echo site_url('wilayah/get_kota'); ?>"
   });

   $("#kecamatan").remoteChained({
      parents : "#kota",
      url : "<?php echo site_url('wilayah/get_kecamatan'); ?>"
   });
